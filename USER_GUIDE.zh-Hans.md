...
- **自定义字段**——除了内置字段之外，你还可以直接在采集或编辑表单中，
  ...
  字段）。
- **提醒**——把某个自定义字段的类型设为"提醒"（续保日期、保修到期，
  任何需要跟进的事情），Dossiary 会在到期时通知你——可以在打开文档库
  时自动提示，也可以随时点击工具栏中的**🔔 检查提醒**查看；如果还不想
  处理，可以先推迟。想要快速设置一次性提醒而不必先配置字段，直接右键
  点击任意文档，选择**添加提醒**即可。一切都不会在后台运行——Dossiary
  只有在你真正打开它，或点击按钮时才会检查。
