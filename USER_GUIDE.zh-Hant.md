...
- **自定義字段**——除了內置字段之外，你還可以直接在採集或編輯表單中，
  ...
  字段）。
- **提醒**——把某個自定義字段的類型設爲"提醒"（續保日期、保修到期，
  任何需要跟進的事情），Dossiary 會在到期時通知你——可以在打開文檔庫
  時自動提示，也可以隨時點擊工具欄中的**🔔 檢查提醒**查看；如果還不想
  處理，可以先推遲。想要快速設置一次性提醒而不必先配置字段，直接右鍵
  點擊任意文檔，選擇**添加提醒**即可。一切都不會在後臺運行——Dossiary
  只有在你真正打開它，或點擊按鈕時纔會檢查。
