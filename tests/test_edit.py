...
async def main():
    async with async_playwright() as p:
        ...
        person_val = await page.locator('#e-person').input_value()
        ...
        await page.fill('#e-person', 'Arne, Lydia')
        ...
        print("--- persisted document_people ---", persisted['document_people'])
        ...
        sidecar = await page.evaluate("""
            (async () => {
                ...
                const fh = await filesDir.getFileHandle('1_editdoc.txt');
                const f = await fh.getFile();
                return await f.text();
            })()
        """)
